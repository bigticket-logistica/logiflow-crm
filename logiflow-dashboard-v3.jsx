...
const Spinner = () => (
  <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:60}}>
    <div style={{width:32,height:32,border:"3px solid #1e3a5f",borderTopColor:"#3B82F6",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
  </div>
);
...
const Modal = ({ title, onClose, children }) => (
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}}>
    <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:14,padding:28,width:560,maxHeight:"90vh",overflow:"auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div style={{fontSize:16,fontWeight:900,color:"#f1f5f9",fontFamily:"'Outfit',sans-serif"}}>{title}</div>
        <button onClick={onClose} style={{background:"#1e293b",border:"1px solid #334155",color:"#94a3b8",borderRadius:8,width:32,height:32,cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
      </div>
      {children}
    </div>
  </div>
...
const Input = ({ value, onChange, placeholder, type="text", style={} }) => (
  <input type={type} value={value} onChange={onChange} placeholder={placeholder}
    style={{width:"100%",background:"#070e1a",color:"#e2e8f0",border:"1px solid #1e3a5f",
...
const Label = ({ children }) => (
  <div style={{fontSize:9,color:"#475569",fontWeight:800,letterSpacing:1,
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <div style={{fontSize:16,fontWeight:800,color:"#f1f5f9",fontFamily:"'Outfit',sans-serif"}}>
      ...
      <div style={{display:"flex",gap:8}}>
        {CANALES_PLANTILLA.map(c=>(
          <button key={c} onClick={()=>setFiltroCanal(c)}
            style={{padding:"6px 14px",borderRadius:20,border:`1px solid ${filtroCanal===c?canalColor(c):"#1e3a5f"}`,
      ...
      {loading ? <Spinner/> : (
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12}}>
          {filtradas.map(p=>{
            ...
            return (
              <div key={p.id} style={{background:"#0f172a",border:`1px solid ${p.activa?color+"33":"#1e293b"}`,
                borderRadius:12,padding:16,opacity:p.activa?1:.6,transition:"all .2s"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:800,color:"#f1f5f9"}}>{p.nombre}</div>
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    <button onClick={()=>toggleActiva(p)}
                      style={{background:p.activa?"#10B98122":"#1e293b",color:p.activa?"#10B981":"#475569",
                        border:`1px solid ${p.activa?"#10B98144":"#334155"}`,borderRadius:6,
                ...
                {p.asunto && (
                  <div style={{fontSize:11,color:"#475569",marginBottom:6}}>
                    <span style={{color:"#334155"}}>Asunto: </span>{p.asunto}
                  </div>
                )}

                <div style={{background:"#070e1a",borderRadius:8,padding:10,marginBottom:10,
          ...
          {filtradas.length===0 && (
            <div style={{gridColumn:"1/-1",textAlign:"center",padding:40,color:"#334155",fontSize:13}}>
      ...
      <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:12,padding:14}}>
        <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:1,marginBottom:8,textTransform:"uppercase"}}>
          Variables disponibles en plantillas
        </div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {VARS.map(v=>(
            <span key={v} style={{background:"#1e293b",color:"#3B82F6",border:"1px solid #3B82F633",
              ...
              <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
                {VARS.map(v=>(
                  <button key={v} onClick={()=>setForm(p=>({...p,contenido:p.contenido+v}))}
                    style={{background:"#1e293b",color:"#3B82F6",border:"1px solid #3B82F633",
...
const ConfigScoreView = () => {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{background:"linear-gradient(135deg,#0f2040,#0b1426)",border:"1px solid #1e3a5f",
        borderRadius:12,padding:16,display:"flex",gap:20,alignItems:"center"}}>
        <div>
          <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:1,textTransform:"uppercase"}}>
      ...
      {loading ? <Spinner/> : (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {dimensiones.map(dim=>{
            ...
            return (
              <div key={dim.id} style={{background:"#0f172a",border:`1px solid ${dim.activa?"#1e3a5f":"#1e293b"}`,
                ...
                <div style={{padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",
                  ...
                  <div>
                    <div style={{fontSize:13,fontWeight:800,color:"#f1f5f9"}}>{dim.nombre}</div>
                    {dim.descripcion && <div style={{fontSize:11,color:"#475569",marginTop:2}}>{dim.descripcion}</div>}
                  </div>
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    <div style={{textAlign:"center"}}>
                      <div style={{fontSize:18,fontWeight:900,color:getScoreColor(maxPts),fontFamily:"monospace"}}>{maxPts}</div>
                      <div style={{fontSize:8,color:"#334155"}}>MAX PTS</div>
                    </div>
                    <button onClick={()=>toggleDim(dim)}
                      style={{background:dim.activa?"#10B98122":"#1e293b",color:dim.activa?"#10B981":"#475569",
                        border:`1px solid ${dim.activa?"#10B98144":"#334155"}`,borderRadius:6,
                ...
                <div style={{padding:12,display:"flex",flexDirection:"column",gap:6}}>
                  {(dim.reglas||[]).map((regla,idx)=>(
                    <div key={idx} style={{display:"flex",alignItems:"center",gap:10,
                      background:"#070e1a",borderRadius:8,padding:"8px 12px"}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:12,color:"#e2e8f0",fontWeight:600}}>{regla.etiqueta}</div>
                        {regla.palabras_clave?.length>0 && (
                          <div style={{display:"flex",gap:4,marginTop:4,flexWrap:"wrap"}}>
                            {regla.palabras_clave.map(k=>(
                              <span key={k} style={{background:"#1e293b",color:"#64748b",border:"1px solid #334155",
                                borderRadius:4,padding:"1px 6px",fontSize:9,fontFamily:"monospace"}}>{k}</span>
                            ))}
                          </div>
                        )}
                        {regla.palabras_clave?.length===0 && (
                          <div style={{fontSize:9,color:"#334155",marginTop:2}}>Sin palabras clave (condición por defecto)</div>
                        )}
                      </div>
                      <div style={{display:"flex",gap:8,alignItems:"center"}}>
                        <div style={{textAlign:"center",minWidth:40}}>
                          ...
                          <div style={{fontSize:8,color:"#334155"}}>PTS</div>
                        </div>
                  ...
                  {(dim.reglas||[]).length===0 && (
                    <div style={{textAlign:"center",color:"#334155",fontSize:11,padding:10}}>
      ...
      {modalRegla && (
        <Modal title={editandoReglaIdx!==null?"Editar regla":"Nueva regla"} onClose={()=>setModalRegla(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:"#070e1a",borderRadius:8,padding:10,fontSize:11,color:"#475569"}}>
              Dimensión: <span style={{color:"#f1f5f9",fontWeight:700}}>{modalRegla.nombre}</span>
            </div>
            <div>
    ...
    <div draggable onDragStart={(e)=>onDragStart(e,lead)} onClick={()=>onSelect(lead)}
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:12,fontWeight:700,color:"#f1f5f9",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{lead.nombre||"Sin nombre"}</div>
          <div style={{fontSize:10,color:"#475569",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{lead.empresa||"Sin empresa"}</div>
        </div>
  ...
  return (
    <div style={{width:ETAPAS_CIERRE.includes(etapa)?220:245,flexShrink:0}}
      ...
      <div style={{padding:"8px 12px",background:"#0f172a",borderRadius:"10px 10px 0 0",
        ...
        <span style={{fontSize:10,fontWeight:800,color:isDragOver?cfg.color:"#64748b",textTransform:"uppercase",letterSpacing:1}}>{cfg.icon} {etapa}</span>
      ...
      <div style={{minHeight:400,background:isDragOver?cfg.color+"08":"#070e1a",borderRadius:"0 0 10px 10px",
  ...
  return (
    <div style={{overflowX:"auto",paddingBottom:12}}>
      <div style={{display:"flex",gap:10,minWidth:"fit-content"}}>
        ...
        <div style={{width:2,background:"linear-gradient(to bottom,transparent,#334155,transparent)",borderRadius:4,flexShrink:0,margin:"0 4px"}}/>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
        {[["🏆","Canal más eficaz",canalEficaz,"#10B981"],["⚠️","Canal menos eficaz",canalMenos,"#EF4444"],["📊","Mayor volumen",canalVol,"#3B82F6"]].map(([icon,label,canal,color])=>{
          ...
          return(
            <div key={label} style={{background:"#0f172a",border:`1px solid ${color}22`,borderRadius:12,padding:16}}>
              <div style={{fontSize:11,fontWeight:800,color:"#475569",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>{icon} {label}</div>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                <span style={{fontSize:24}}>{cfg.icon}</span>
                <div><div style={{fontSize:16,fontWeight:800,color:cfg.color}}>{cfg.label}</div><div style={{fontSize:11,color:"#475569"}}>{canal.total} leads</div></div>
              </div>
              <div style={{display:"flex",gap:16}}>
                <div style={{textAlign:"center"}}><div style={{fontSize:22,fontWeight:900,color,fontFamily:"monospace"}}>{canal.eficacia}%</div><div style={{fontSize:9,color:"#475569"}}>EFICACIA</div></div>
                <div style={{textAlign:"center"}}><div style={{fontSize:22,fontWeight:900,color:"#10B981",fontFamily:"monospace"}}>{canal.ganados}</div><div style={{fontSize:9,color:"#475569"}}>GANADOS</div></div>
              </div>
            </div>
          );
        })}
      ...
      <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:12,padding:16}}>
        <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:2,marginBottom:14,textTransform:"uppercase"}}>Tasa de conversión por canal</div>
        {canalStats.map(({canal,total,ganados:g,eficacia})=>{const cfg=getCanalCfg(canal);return(
          <div key={canal} style={{marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              ...
              <div style={{display:"flex",gap:12}}><span style={{fontSize:10,color:"#475569"}}>{total} leads · {g} ganados</span><span style={{fontSize:12,fontWeight:800,color:cfg.color}}>{eficacia}%</span></div>
            </div>
            <div style={{height:6,background:"#1e293b",borderRadius:4}}><div style={{height:"100%",width:`${eficacia}%`,background:cfg.color,borderRadius:4,transition:"width .8s ease"}}/></div>
          </div>
        );})}
      </div>
      <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:12,padding:16}}>
        <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:2,marginBottom:14,textTransform:"uppercase"}}>Score promedio por etapa</div>
        {scoreEtapa.map(({etapa,score,count})=>{const cfg=ETAPA_CFG[etapa]||{color:"#64748b",icon:"•"};return(
          <div key={etapa} style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
            <div style={{width:150,fontSize:11,color:"#64748b",flexShrink:0}}>{cfg.icon} {etapa}</div>
            <div style={{flex:1,height:6,background:"#1e293b",borderRadius:4}}><div style={{height:"100%",width:`${score}%`,background:getScoreColor(score),borderRadius:4,transition:"width .8s ease"}}/></div>
...
const LeadPanel = ({ lead, onClose, onUpdate }) => {
  ...
  return (
    <div style={{position:"fixed",top:0,right:0,width:480,height:"100vh",background:"#0b1426",borderLeft:"1px solid #1e3a5f",display:"flex",flexDirection:"column",zIndex:100,boxShadow:"-30px 0 80px rgba(0,0,0,.7)"}}>
      <div style={{padding:"18px 20px",background:"linear-gradient(135deg,#0f1f3d,#0b1426)",borderBottom:"1px solid #1e3a5f",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:1}}>
            <div style={{fontSize:18,fontWeight:800,color:"#f1f5f9"}}>{lead.nombre||"Sin nombre"}</div>
            <div style={{fontSize:12,color:"#64748b",marginTop:2}}>{lead.cargo?`${lead.cargo} · `:""}{lead.empresa||"Sin empresa"}</div>
        ...
        <div style={{marginTop:12,display:"flex",gap:8,alignItems:"center"}}>
          <select value={etapa} onChange={e=>handleEtapaChange(e.target.value)}
            style={{flex:1,background:"#0f172a",color:etapaCfg.color,border:`1px solid ${etapaCfg.color}44`,borderRadius:8,padding:"7px 12px",fontSize:12,cursor:"pointer",fontWeight:600}}>
        ...
        {tab==="info"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:10,padding:14}}>
              <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:1,marginBottom:10,textTransform:"uppercase"}}>Datos de contacto</div>
            ...
            {lead.notas&&<div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:10,padding:14}}><div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:1,marginBottom:8,textTransform:"uppercase"}}>Notas</div><div style={{fontSize:12,color:"#94a3b8",lineHeight:1.6}}>{lead.notas}</div></div>}
          </div>
        )}
        {tab==="score"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{background:"#0f172a",border:`1px solid ${scoreColor}33`,borderRadius:10,padding:16,textAlign:"center"}}>
            ...
            {lead.razones_score&&<div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:10,padding:14}}>
              <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:1,marginBottom:10,textTransform:"uppercase"}}>Detalle del score</div>
              {lead.razones_score.split(" | ").map((r,i)=>(
                <div key={i} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"6px 0",borderBottom:"1px solid #1e293b"}}>
                  <span style={{color:"#10B981"}}>✓</span><span style={{fontSize:12,color:"#94a3b8"}}>{r}</span>
                </div>
              ))}
            </div>}
          </div>
          ...
          <div>
            {[{icon:"🎯",label:`Captado vía ${lead.canal||"desconocido"}`,fecha:formatFecha(lead.created_at),color:"#3B82F6"},
              ...
              <div key={i} style={{display:"flex",gap:12,padding:"12px 0",borderBottom:"1px solid #1e293b"}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:12,color:"#e2e8f0",fontWeight:600}}>{ev.label}</div><div style={{fontSize:10,color:"#475569",marginTop:2}}>{ev.fecha}</div></div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:12,padding:16}}>
          <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:2,marginBottom:14,textTransform:"uppercase"}}>Leads por canal</div>
          {canalData.map(([canal,count])=>{const cfg=getCanalCfg(canal);return(
            <div key={canal} style={{marginBottom:10}}>
              ...
              <div style={{height:5,background:"#1e293b",borderRadius:4}}><div style={{height:"100%",width:`${(count/leads.length)*100}%`,background:cfg.color,borderRadius:4,transition:"width .8s ease"}}/></div>
            </div>
          );})}
        </div>
        <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:12,padding:16}}>
          <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:2,marginBottom:14,textTransform:"uppercase"}}>Pipeline por etapa</div>
          {etapaData.map(({etapa,count})=>{const cfg=ETAPA_CFG[etapa]||{color:"#64748b"};return(
            <div key={etapa} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #1e293b"}}>
      ...
      <div style={{background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:12,padding:16}}>
        <div style={{fontSize:10,fontWeight:800,color:"#475569",letterSpacing:2,marginBottom:12,textTransform:"uppercase"}}>Leads recientes</div>
        {leads.slice(0,5).map(lead=>(
          <div key={lead.id} style={{display:"flex",alignItems:"center",gap:12,padding:"9px 0",borderBottom:"1px solid #0f172a"}}>
            <div style={{width:34,height:34,borderRadius:"50%",background:"#1e293b",border:"1px solid #334155",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#64748b",fontWeight:700,flexShrink:0}}>{(lead.nombre||"?")[0].toUpperCase()}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:12,fontWeight:700,color:"#f1f5f9",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{lead.nombre} · {lead.empresa}</div>
              <div style={{fontSize:10,color:"#475569"}}>{lead.etapa||"Nuevo Lead"} · {formatFecha(lead.created_at)}</div>
            </div>
...
const TablaLeads = ({ leads, onSelect }) => (
  <div style={{background:"#0f172a",borderRadius:12,border:"1px solid #1e3a5f",overflow:"hidden"}}>
    <table style={{width:"100%",borderCollapse:"collapse"}}>
      <thead><tr style={{background:"#070e1a"}}>{["Prospecto","Empresa","Canal","Etapa","Score","Clasificación","Actualizado",""].map(h=><th key={h} style={{padding:"10px 14px",textAlign:"left",fontSize:9,fontWeight:800,color:"#334155",textTransform:"uppercase",letterSpacing:1}}>{h}</th>)}</tr></thead>
      <tbody>{leads.map((lead,i)=>{const cfg=ETAPA_CFG[lead.etapa]||{color:"#64748b",icon:"•"};return(
        <tr key={lead.id} style={{borderTop:"1px solid #0f172a",background:i%2===0?"#0f172a":"#0b1426",cursor:"pointer"}}
          onClick={()=>onSelect(lead)} onMouseEnter={e=>e.currentTarget.style.background="#0f2040"} onMouseLeave={e=>e.currentTarget.style.background=i%2===0?"#0f172a":"#0b1426"}>
          <td style={{padding:"10px 14px"}}><div style={{fontSize:12,fontWeight:700,color:"#f1f5f9"}}>{lead.nombre||"—"}</div><div style={{fontSize:10,color:"#334155"}}>{lead.email||"—"}</div></td>
          ...
          <td style={{padding:"10px 14px"}}>{lead.clasificacion?<Tag label={`${lead.emoji||""} ${lead.clasificacion}`} color={lead.clasificacion?.includes("Caliente")?"#EF4444":lead.clasificacion?.includes("Candidato")?"#F59E0B":"#F97316"}/>:<span style={{color:"#334155",fontSize:11}}>—</span>}</td>
          <td style={{padding:"10px 14px",fontSize:11,color:"#334155"}}>{formatFecha(lead.updated_at)}</td>
          <td style={{padding:"10px 14px"}}><button onClick={e=>{e.stopPropagation();onSelect(lead);}} style={{background:"#1e3a5f",color:"#3B82F6",border:"none",borderRadius:6,padding:"5px 10px",fontSize:11,cursor:"pointer",fontWeight:700}}>Ver →</button></td>
        </tr>
      );})}</tbody>
    </table>
  </div>
...
export default function App() {
  ...
  const filtered=leads.filter(l=>(l.nombre||"").toLowerCase().includes(busqueda.toLowerCase())||(l.empresa||"").toLowerCase().includes(busqueda.toLowerCase())||(l.email||"").toLowerCase().includes(busqueda.toLowerCase()));
  ...
  const NAV=[
    ...
    {id:"config_score",icon:"⚙️",label:"Config Score"},
  ];

  return (
    <div style={{display:"flex",height:"100vh",background:"#070e1a",color:"#e2e8f0",overflow:"hidden",fontFamily:"'DM Mono','Fira Code',monospace"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:#070e1a}::-webkit-scrollbar-thumb{background:#1e3a5f;border-radius:4px}
        input,textarea,select{outline:none;font-family:inherit;}
        @keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}
      `}</style>

      {/* SIDEBAR */}
      <div style={{width:200,background:"#0b1426",borderRight:"1px solid #0f2040",display:"flex",flexDirection:"column",flexShrink:0}}>
          ...
          {NAV.map(item=>(
            <button key={item.id} onClick={()=>setSeccion(item.id)}
              style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:8,
                background:seccion===item.id?"#0f2040":"none",border:seccion===item.id?"1px solid #1e3a5f":"1px solid transparent",
              ...
              {item.label}
              {item.id==="leads"&&<span style={{marginLeft:"auto",background:"#3B82F622",color:"#3B82F6",fontSize:9,fontWeight:800,padding:"1px 6px",borderRadius:20}}>{leads.length}</span>}
            </button>
          ))}
        </nav>
        <div style={{padding:12,borderBottom:"1px solid #0f2040"}}>
          <button onClick={fetchLeads} style={{width:"100%",background:"#0f2040",border:"1px solid #1e3a5f",color:"#64748b",borderRadius:8,padding:"7px",fontSize:11,cursor:"pointer",fontWeight:600,fontFamily:"'Outfit',sans-serif"}}>🔄 Actualizar</button>
          {lastUpdate&&<div style={{fontSize:9,color:"#334155",textAlign:"center",marginTop:4}}>{lastUpdate.toLocaleTimeString("es-CL",{hour:"2-digit",minute:"2-digit"})}</div>}
        </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{height:56,background:"#0b1426",borderBottom:"1px solid #0f2040",display:"flex",alignItems:"center",padding:"0 20px",gap:12,flexShrink:0}}>
          <div style={{flex:1,display:"flex",gap:10,alignItems:"center"}}>
            {seccion==="leads"&&(
              <>
                <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="🔍 Buscar leads..."
                  style={{background:"#0f172a",color:"#e2e8f0",border:"1px solid #1e3a5f",borderRadius:7,padding:"6px 12px",fontSize:12,width:220}}/>
                <div style={{display:"flex",background:"#0f172a",border:"1px solid #1e3a5f",borderRadius:7,overflow:"hidden"}}>
                  {[["pipeline","⬛ Kanban"],["lista","☰ Lista"]].map(([v,label])=>(
                    <button key={v} onClick={()=>setVista(v)} style={{padding:"6px 12px",background:vista===v?"#1e3a5f":"none",border:"none",color:vista===v?"#f1f5f9":"#475569",cursor:"pointer",fontSize:11,fontWeight:vista===v?700:400}}>{label}</button>
                  ))}
                </div>
              </>
            )}
            <div style={{fontSize:11,color:"#1e3a5f"}}>
              ...
              {seccion==="config_score"&&"⚙️ Configuración del score"}
            </div>
          </div>
          <div style={{background:"#0d2b1a",border:"1px solid #25D36633",borderRadius:7,padding:"5px 10px",fontSize:10,color:"#25D366",fontWeight:700}}>⚡ N8N Activo</div>
        </div>

        <div style={{flex:1,overflow:"auto",padding:20}}>
          {loading?<Spinner/>:error?(
            <div style={{background:"#1a0a0a",border:"1px solid #EF444433",borderRadius:12,padding:24,textAlign:"center"}}>
              <div style={{fontSize:24,marginBottom:8}}>⚠️</div>
              <div style={{fontSize:14,color:"#EF4444",fontWeight:700}}>{error}</div>
              <button onClick={fetchLeads} style={{marginTop:12,background:"#1e293b",color:"#94a3b8",border:"1px solid #334155",borderRadius:8,padding:"8px 16px",fontSize:12,cursor:"pointer"}}>Reintentar</button>
            </div>
          ):(
            <>
              {seccion==="dashboard"&&<DashboardMetrics leads={leads}/>}
              {seccion==="leads"&&(vista==="pipeline"?<Pipeline leads={filtered} onSelect={setSelectedLead} onEtapaChange={handleEtapaChange}/>:<TablaLeads leads={filtered} onSelect={setSelectedLead}/>)}
              {seccion==="kpis"&&<KPIsView leads={leads}/>}
              {seccion==="plantillas"&&<PlantillasView/>}
              {seccion==="config_score"&&<ConfigScoreView/>}
            </>
          )}
        </div>
      </div>
